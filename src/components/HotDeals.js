...
import { backendUrlGetHotDeals } from "../BackendURL";
...
class HotDeals extends Component {
    ...
    fetHotDeals = () => {
        /*
           Make an axios GET request to fetch the hot deals available
        */
        axios.get(backendUrlGetHotDeals).then(res => {
            this.setState({ hotDeals: res.data })
        }).catch(err => {
            if (err.response) {
                this.setState({ hotDeals: [] })
            }
            else {
                this.setState({ hotDeals: [] })
            }
    ...
    /**
     * Rendering the hotDeals package
        ...
        return (
            <div className="mt-5" style={{display:'flex',justifyContent:'center', flexWrap:'wrap'}}>
                {this.state.hotDeals.length > 0 ?
                    this.state.hotDeals.map(singlePackage => <ViewPackage key={singlePackage.destinationId} package={singlePackage} />) :
                    <h1 className="text-danger">Sorry No Hot Deals are available right now</h1>}
            </div>
        )
    }
